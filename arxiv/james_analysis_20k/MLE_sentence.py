...
def df_to_val_set(df, alpha=None):
    ...
    human_data = human_data[human_data['human_index'] >= num_abs * .75]
    ai_data = ai_data[ai_data['ai_index'] >= num_abs * .75]

    # import pdb; pdb.set_trace()
    assert(set(human_data['human_index']) == set(ai_data['ai_index']))
    min_abs = min(human_data['human_index'])
    if alpha is not None:
        ai_subset = ai_data[ai_data['ai_index'] < min_abs + alpha * (num_abs - min_abs)]
        human_subset = human_data[human_data['human_index'] >= min_abs + alpha * (num_abs - min_abs)]
    else:
        ai_subset = ai_data
...
if __name__ == "__main__":
    ...
    for val_year in test_years:
        logprob_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_year}_logprob_{category}_{subsample_size}.parquet"
        ...
        val_data = pd.read_parquet(f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{val_year}_ai_{category}_{subsample_size}_sentence.parquet")
    ...
    plt.savefig(f"mle_sentence_{train_year}.pdf",format='pdf')
    plt.clf()
