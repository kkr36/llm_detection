...
if __name__ == "__main__":
    ...
    for year in tqdm(years):
        ...
        #     continue
        # train_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_start_year}_{train_end_year}_ai_{category}_{subsample_size}.parquet" if train_start_year != train_end_year else f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_end_year}_ai_{category}_{subsample_size}.parquet"
        # train_path = train_path.replace(str(subsample_size), str(subsample_size // 2)) if not os.path.exists(train_path) else train_path
        train_data = pd.read_parquet(f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{year}_ai_{category}_{subsample_size}_abstract.parquet")
        train_data = train_data.iloc[:int(len(train_data) * .75)]
        ...
        # import pdb; pdb.set_trace()
        # assert(set(human_data['human_index']) == set(ai_data['ai_index']))
        # num_abs = max(human_data['human_index']) + 1
        # human_train = human_data[human_data['human_index'] < num_abs*.75]
        # ai_train = ai_data[ai_data['ai_index'] < num_abs*.75]
        ai_subset, human_subset = train_data[['ai_sentence']], train_data[['human_sentence']]
        # ai_subset['inference_sentence'] = ai_subset['ai_sentence']
        # human_subset['inference_subset'] = human_subset['human_sentence']


        estimate_text_distribution(human_subset, ai_subset, f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_start_year}_logprob_{category}_{subsample_size}_abstract.parquet")