...
def df_to_val_set(df, alpha=None):
    human_data = df[["human_sentence"]]
    ai_data = df[["ai_sentence"]]

    human_data = human_data.iloc[int(len(human_data) * .75):].reset_index()
    ai_data = ai_data.iloc[int(len(ai_data) * .75):].reset_index()

    # import pdb; pdb.set_trace()
    if alpha is not None:
        ai_subset = ai_data.iloc[:int(len(ai_data) * alpha)]
        human_subset = human_data.iloc[int(len(human_data) * alpha):]
        assert(len(ai_subset) + len(human_subset) == len(human_data) and len(human_data) == len(ai_data))
    else:
        ai_subset = ai_data
...
if __name__ == "__main__":
    ...
    train_year = 2020
    logprob_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_year}_logprob_{category}_{subsample_size}_abstract.parquet"
    mle = MLE(logprob_path)

    test_years = [2020, 2023, 2025]
    # import pdb; pdb.set_trace()
    sols = []
    ...
    for test_year in test_years:
        ...
        val_data = pd.read_parquet(f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{test_year}_ai_{category}_{subsample_size}_abstract.parquet")
        val_data = df_to_val_set(val_data, 0)

        solution, half_width = mle.inference(val_data,True)
        print(f"{test_year}: {solution} += {half_width}")
        sols.append(solution)
        
        # Make a line plot
        plt.plot(test_years[:len(sols)], sols, marker='o')

    # Add labels
    # plt.legend(title="Train Year", loc="best")
    plt.xlabel("Year")
    plt.ylabel("Pct predicted AI")
    plt.savefig(f"mle.pdf",format='pdf')
    plt.clf()
