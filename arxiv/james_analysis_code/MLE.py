...
class MLE():
    ...
    def bootstrap_alpha_inference(self,data,inference_data,n_bootstrap=1000):
    ...
    def inference(self,inference_data,exploded_data=False):
        """
        ...
        confidence_interval=self.bootstrap_alpha_inference(data, inference_data)
...
def df_to_val_set(df, alpha=None):
    ...
    ai_subset = ai_data.iloc[:int(min_size * alpha)]
    human_subset = human_data[int(min_size * alpha):]
    # import pdb; pdb.set_trace()
    ai_subset["inference_sentence"] = ai_subset["ai_sentence"]
    human_subset["inference_sentence"] = human_subset["human_sentence"]
    return pd.concat([ai_subset[["inference_sentence"]], human_subset[["inference_sentence"]]]) if alpha != 0 else pd.DataFrame(human_subset["inference_sentence"]) if alpha != 1 else pd.DataFrame(ai_subset["inference_sentence"])

if __name__ == "__main__":

    splits = ['train', 'test_sample']
    data_sizes = [6000, 1000]
    val_sizes = [3000, 1000]
    vals = ['validation', 'test_sample']
    alphas = [0, .33, .66, 1]
    results = []

    for split, data_size in zip(splits, data_sizes):

        logprob_path = f"/share/garg/kkr36/Task_A/{split}_logprob_{data_size}.parquet"
        mle = MLE(logprob_path)

        for val, val_size in zip(vals, val_sizes):
            for alpha in alphas:
                val_data = pd.read_parquet(f"/share/garg/kkr36/Task_A/{val}_tokenized_{val_size}.parquet")
                ...
                results.append({
                    ...
                    'lower_bound': solution - half_width,
                    'upper_bound': solution + half_width
                })

    results_df = pd.DataFrame(results)

    # ============================================================================
    # VISUALIZATION 1: Heatmap of solutions by alpha and validation set
    # ============================================================================
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    for idx, split in enumerate(splits):
        data = results_df[results_df['train_split'] == split]
        ...
        sns.heatmap(pivot, annot=True, fmt='.3f', cmap='RdYlGn', 
                    ax=axes[idx], cbar_kws={'label': 'Solution Value'})
        axes[idx].set_title(f'Training Split: {split}')
        axes[idx].set_xlabel('Validation Split')
        axes[idx].set_ylabel('Alpha')

    plt.tight_layout()
    plt.savefig('heatmap_by_split_alpha.png', dpi=300, bbox_inches='tight')
    plt.clf(); plt.close()

    # ============================================================================
    # VISUALIZATION 2: Line plots showing alpha effect with confidence intervals
    # ============================================================================
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    ...
    for idx, split in enumerate(splits):
        ...
        for val in vals:
            subset = data[data['val_split'] == val]
            ax.plot(subset['alpha'], subset['solution'], marker='o', label=val, linewidth=2)
            ax.fill_between(subset['alpha'], 
                            subset['lower_bound'], 
                            subset['upper_bound'], 
                            alpha=0.2)
        ...
        ax.set_title(f'Training Split: {split}', fontsize=14)
        ax.legend()
        ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('alpha_effect_with_ci.png', dpi=300, bbox_inches='tight')
    plt.clf(); plt.close()

    # ============================================================================
    # VISUALIZATION 3: Faceted grid showing all combinations
    # ============================================================================
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()

    for idx, (split, val) in enumerate([(s, v) for s in splits for v in vals]):
        ax = axes[idx]
        data = results_df[(results_df['train_split'] == split) & 
                        (results_df['val_split'] == val)]
        
        ax.bar(range(len(alphas)), data['solution'], 
            yerr=data['half_width'], capsize=5, alpha=0.7)
        ax.set_xticks(range(len(alphas)))
        ax.set_xticklabels([f'{a:.2f}' for a in alphas])
        ax.set_xlabel('Alpha')
        ax.set_ylabel('Solution Value')
        ax.set_title(f'Train: {split} | Val: {val}')
        ax.grid(True, alpha=0.3, axis='y')

    plt.tight_layout()
    plt.savefig('faceted_bar_plots.png', dpi=300, bbox_inches='tight')
    plt.clf(); plt.close()

    # ============================================================================
    # VISUALIZATION 4: Uncertainty (half_width) comparison
    # ============================================================================
    ...
    for idx, split in enumerate(splits):
        for jdx, val in enumerate(vals):
            offset = (idx * len(vals) + jdx - 1.5) * width / 2
            data = results_df[(results_df['train_split'] == split) & 
                            (results_df['val_split'] == val)]
            
            ax.bar(x + offset, data['half_width'], width/2, 
                label=f'{split[:5]}-{val[:5]}', alpha=0.8)

    ax.set_xlabel('Alpha', fontsize=12)
    ax.set_ylabel('Confidence Interval Half-Width', fontsize=12)
    ax.set_title('Uncertainty Across Configurations', fontsize=14)
    ax.set_xticks(x)
    ax.set_xticklabels([f'{a:.2f}' for a in alphas])
    ax.legend()
    ax.grid(True, alpha=0.3, axis='y')
    ...
    # ============================================================================
    # VISUALIZATION 5: Summary statistics table
    # ============================================================================
    summary = results_df.groupby(['train_split', 'val_split', 'alpha']).agg({
        'solution': ['mean', 'std'],
        'half_width': 'mean'
    }).round(4)
