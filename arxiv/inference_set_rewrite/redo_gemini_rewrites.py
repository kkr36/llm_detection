...
from gemini_api import call_gemini_2_flash, call_gemini_2_flash_lite

def skip_model(*args, **kwargs):
    # Return shape-compatible output
    return None

query_fns = [call_gemini_2_flash_lite, skip_model, call_gemini_2_flash, skip_model]
# query_fns = [openai_oss_query] * 4
llm_labels = ["Gemini 2.0 Flash-Lite", "Gemini 3 Preview", "Gemini 2.0 Flash", "Gemini 2.5 Flash"]
assert(len(query_fns) == len(llm_labels))

def rewrite_abstract(prompt_model, abstract, model_name):

    if prompt_model is skip_model:
        # return None, model_name
        existing_path = f"/share/garg/arxiv_kaggle/multillm/data_raw/arxiv_2020_ai_{category}_{subsample_size//2}_fronthalf.parquet"
        existing_abstracts = pd.read_parquet(existing_path)

        mask = (
            (existing_abstracts["human_abstract"] == abstract)
            & (existing_abstracts[model_name].notna())
            & (existing_abstracts[model_name].str.len() > 0)
        )

        matches = existing_abstracts.loc[mask]
        # import pdb; pdb.set_trace()
        # assert(len(matches==1))
        try:
            assert(len(matches==1))
            res = matches.iloc[0][model_name]
...
if __name__ == "__main__":
    ...
    for year in tqdm(train_years):
      fpath = f"/share/garg/arxiv_kaggle/multillm/data_raw/arxiv_{year}_ai_{category}_{subsample_size//2}_fronthalf_gemini.parquet"
      ...
      for i in range(len(arxiv_data)):
          ...
          if llm_abs is None or len(llm_abs) == 0:
            #   new_abs = rewrite_abstract(query_fns[llms[i]], orig_abs, llm_labels[llms[i]])
              import pdb; pdb.set_trace()
              changed = True

            #   arxiv_data.at[i, llm_labels[llms[i]]] = new_abs[0]

              print(i)
              # print(llm_abs)
              # print(orig_abs)
              # print(llm_labels[llms[i]])
      if changed:
        import pdb; pdb.set_trace()
        arxiv_data.to_parquet(fpath)
