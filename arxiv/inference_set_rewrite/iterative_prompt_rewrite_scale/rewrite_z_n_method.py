...
if __name__ == "__main__":
    ...
    iteration = 2 # 0 --> None
    method = "PN"
    strategies = [("Z", z_mapping[method][iteration])]
    full_col_name = f"Z_{iteration}_{method}"

    abstract_col = "human_abstract"

    for year in tqdm(train_years):

        arxiv_path = f"/share/garg/arxiv_kaggle/multillm/data_raw/arxiv_{year}_xyz_{category}_{subsample_size//2}_fronthalf.parquet"
        ...
        arxiv_data[f"rewrite_{full_col_name}"] = ai_writing_Z
        # import pdb; pdb.set_trace()
        arxiv_data.to_parquet(arxiv_path)
