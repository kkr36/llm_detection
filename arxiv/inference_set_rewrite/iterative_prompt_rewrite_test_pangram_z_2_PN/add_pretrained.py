...
split = args.split
pangram = args.pangram
to_rewrite = args.to_rewrite
timestep = args.timestep
input_csv = f"results_{timestep}_oss_{split}_{to_rewrite}"
input_data = pd.read_csv(f"{input_csv}.csv")
source_pretrained = pd.read_csv(f"results_0_oss_{split}_{to_rewrite}_pretrained.csv")
...
if __name__ == "__main__":
    ...
    for seed in range(seeds):
        path_to_pretrained = Path(f"/home/kkr36/llm_detection/arxiv/pu/lipton/PU_learning/logging_accuracy_xy/normal_sentence/alpha_0/{seed}/xzz/xy_3")
    ...
    abstract_dict = {
        "original_score": source_pretrained["original_score"].tolist(),
        "original_score_avg": source_pretrained["original_score_avg"].tolist(),
        f"mirror_{timestep}_score": [],
        f"mirror_{timestep}_score_avg": []
    }

    new_text = input_data[f"mirror_{timestep}"].tolist()

    for i, mirror_t in enumerate(tqdm(new_text)):
        sentences = clean_text(split_into_sentences(mirror_t))
        sentence_scores = []
        for sentence in tqdm(sentences, leave=False):
            avg_score_buffer = []
            for net in nets:
                _, score = individual_predict(net, device, sentence)
                # score = score if  # for PU only
                avg_score_buffer.append(score.cpu())
            sentence_scores.append(np.mean(avg_score_buffer))

        # update dict with sentences, scores
        abstract_dict[f"mirror_{timestep}_score"].append(sentence_scores)
        abstract_dict[f"mirror_{timestep}_score_avg"].append(np.mean(sentence_scores))
