...
if __name__ == "__main__":
    ...
    for year in tqdm(train_years):
      fpath = f"/share/garg/arxiv_kaggle/multillm/double_rewrite/arxiv_{year}_ai_{category}_{subsample_size}_{rewrite_pct}_fronthalf.parquet"
      arxiv_data = pd.read_parquet(fpath)
      llms = [i % len(llm_labels) for i in range(len(arxiv_data))]
      new_llms = [(i+1) % len(llm_labels) for i in range(len(arxiv_data))]
      changed = False
      num_rewrite = int(rewrite_pct * len(arxiv_data))

      for i in range(len(arxiv_data)):
        row = arxiv_data.iloc[i]
        orig_abs = row['human_abstract']
        llm_abs = row[llm_labels[llms[i]]]
        new_llm_abs = row[llm_labels[new_llms[i]]]

        if (new_llm_abs is None or len(new_llm_abs) == 0) and i < num_rewrite:
            # import pdb; pdb.set_trace()
            print(year, i)
            assert(llm_abs is not None and len(llm_abs) > 0)
            new_abs = rewrite_abstract(query_fns[new_llms[i]], llm_abs, llm_labels[new_llms[i]])
            changed = True
            arxiv_data.at[i, llm_labels[new_llms[i]]] = new_abs[0]
