...
to_mirror = 50
timestep = 4
input_csv = f"results_{timestep}_oss_{split}_{to_mirror}_pretrained"
...
if __name__ == "__main__":
    ...
    for seed in range(seeds):
        ...
        path_to_pretrained = Path(f"/home/kkr36/llm_detection/arxiv/pu/lipton/PU_learning/logging_accuracy_xy/normal_sentence/alpha_{0}/{seed}/xy_3")
    ...
    human_text, new_text = input_data["human"].tolist(), input_data[f"mirror_{timestep}"].tolist()
    
    for i, (human_abstract, mirror_t) in enumerate(list(zip(human_text, new_text))):
        for abstract, label in [(human_abstract, "human"), (mirror_t, f"mirror_{timestep}")]:
