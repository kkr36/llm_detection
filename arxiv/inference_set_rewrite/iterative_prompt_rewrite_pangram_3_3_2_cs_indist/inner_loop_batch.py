...
def _selected_split():
    parser = argparse.ArgumentParser(description="Batch-generate abstract mirrors.")
    parser.add_argument("--split", choices=("val", "test"), default="val")
    return parser.parse_args().split


split = _selected_split() if __name__ == "__main__" else "val"
to_rewrite = 50 if split == "val" else 15
output_csv = f"results_{CURRENT_TIMESTEP}_batch_{split}_{to_rewrite}.csv"
...
if __name__ == "__main__":
    ...
    start_idx = test_start if split == "val" else val_start
    assert to_rewrite == (50 if split == "val" else 15)
