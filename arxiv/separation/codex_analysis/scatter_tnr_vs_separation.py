...
Adapted from ../scatter_tnr_vs_separation.py to use the 6-class, all-2020-sourced
embeddings from embed_sentences.py in this directory. The original script read
embeddings built from a 2010 parquet while logging_accuracy_llm.csv's PU-learning
results are all train_year/test_year == 2020 — a latent year mismatch. Here both
sides of the correlation are consistently 2020-sourced.

Codex IS included (as of the logging_accuracy_llm_codex_remade.csv update, which
adds PN-method TNR rows for every Codex pair) — confirmed this is the same CSV
that produces the heatmaps in
pu/lipton/PU_learning/plot_helper/logging_accuracy_llm_codex_remade_paper via
plot_heatmaps_codex.py (output_folder = <csv stem> + "_paper").

Each dot = ordered pair (train_llm=a, test_llm=b), among {GPT OSS 120b, Llama 3.3 70b
Instruct, Gemini 3 Preview, Qwen, Codex}.
  - x-axis (plot 1): centroid distance between a and b
  - x-axis (plot 2): logistic-regression error rate between a and b
  - y-axis (both):   TNR from logging_accuracy_llm_codex_remade.csv (PN method)
"""
...
CSV_PATH  = "/home/kkr36/llm_detection/arxiv/pu/lipton/PU_learning/logging_accuracy_llm_codex_remade.csv"
FIGS_DIR  = "/home/kkr36/llm_detection/arxiv/separation/codex_analysis/figs"
TABLE_OUT = "/share/garg/arxiv_kaggle/lda_2020/tnr_separation_codex.csv"

# All 5 AI LLMs with PU-learning TNR results (skip Human label 0).
LABEL_NAMES = {
    1: "GPT OSS 120b",
    2: "Llama 3.3 70b",
    3: "Gemini 3 Preview",
...
def main():
    ...
    make_scatter(pairs, x_centroid, y_tnr,
                 "Centroid Distance",
                 os.path.join(FIGS_DIR, "scatter_tnr_vs_centroid_dist.pdf"))

    make_scatter(pairs, x_error, y_tnr,
                 "Logistic Regression Error Rate",
                 os.path.join(FIGS_DIR, "scatter_tnr_vs_error_rate.pdf"))
    ...
    make_scatter_averaged(avg_pairs, avg_x_centroid, avg_y_tnr,
                          "Centroid Distance",
                          os.path.join(FIGS_DIR, "scatter_avgtnr_vs_centroid_dist.pdf"))

    make_scatter_averaged(avg_pairs, avg_x_error, avg_y_tnr,
                          "Logistic Regression Error Rate",
                          os.path.join(FIGS_DIR, "scatter_avgtnr_vs_error_rate.pdf"))

    # Save the merged per-pair table used to build the plots
    table = pd.DataFrame({
        "train_llm": [a for a, b in pairs],
        "test_llm":  [b for a, b in pairs],
        "centroid_dist": x_centroid,
        "error_rate": x_error,
        "tnr": y_tnr,
    })
