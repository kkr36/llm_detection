...
# For TEDn: alpha=0.5, flip=True (human=positive)
method_config = {
    "PN":   {"train_alpha": 0,   "flip": False},
    "TEDn": {"train_alpha": 0.25, "flip": True},
}
...
for train_method in train_methods[:1]:
    cfg = method_config[train_method]
    ...
    model_path = None
    for seed in seeds:
        alpha_dir = Path(f"{entrance_path}/normal_sentence/alpha_{train_alpha}/{seed}/xy_{epochs}")
    ...
    print(f"Evaluating {train_method} (flip={flip}, train_alpha={train_alpha}) across {len(nets)} seeds")
    ...
    for seed, net in nets.items():
        pos_probs, unlabeled_probs, unlabeled_targets = get_preds_xy(
            net, device, test_alpha, flip, seed, sentence, clean)
