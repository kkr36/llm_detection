...
#
# Evaluate Pangram 3.3.2 on the HUMAN-sourced rewrite_Z column (prompt #9 applied
# to human_abstract) in the v1 parquet, over the exact same 2,000-row sample
# (seed 42) used for the earlier rewrite_Z Pangram-4 / Pangram-3.2 runs. This
# fills the missing (Pangram 3.3.2 × rewrite_Z) cell. Uses the discounted Bulk API.
#
# Model identifier (this API key only exposes two): Pangram 3.3.2 -> model="default"
# (version 3.3.2), billed per 1,000-word block. "default" currently serves v3.3.2.
# Rows are logged under learning_method "pangram_3.3.2", eval_llm "rewrite_Z".
...
EVAL_COL    = "rewrite_Z"              # human-sourced: prompt #9 applied to human_abstract
...
if __name__ == "__main__":
    # ── 1. load & sample data ─────────────────────────────────────────────────
    # Same v1 parquet + seed 42 as the earlier rewrite_Z runs, so this scores the
    # exact same 2,000 rewrite_Z texts (now under Pangram 3.3.2 instead of 3.2 / 4).
    ...
    for method_label, model_id, words_per_unit in MODELS:
        ...
        raw_out_path = os.path.join(RAW_OUT_DIR, f"{method_label}_{EVAL_COL}_raw.csv")
