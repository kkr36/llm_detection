"""
Evaluate TEDn-trained models (train_llm = X) on eval_llm = rewrite_Z.

This reuses the exact evaluation logic of prepare_optimization.py (the script that
creates logging_accuracy_xz.csv): it loads the per-seed TEDn DistilBert checkpoints
trained on the X rewrite at train_alpha=0.25 (flip=True, human=positive), runs
get_preds_xy on the rewrite_Z unlabeled evaluation column, and computes the same
bootstrapped metric suite. Results are written to a dedicated CSV so the canonical
logging_accuracy_xz.csv is left untouched.
"""
...
# SWITCHES  (mirrors prepare_optimization.py)
...
# The single requested experiment: TEDn model trained on X, evaluated on rewrite_Z.
# TEDn => train_alpha=0.25, flip=True (human = positive), exactly as in
# prepare_optimization.py config line for {"train_method": "TEDn", "llm": "X"}.
configs = [
...
output_csv = "logging_accuracy_xz_TEDn_X_rewrite_Z.csv"
