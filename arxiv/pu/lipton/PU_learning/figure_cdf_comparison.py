...
for year in YEARS:
    ...
    mask = targets == 1
    plot_ecdf(axes[0], 1 - probs[mask], label=str(year), color=YEAR_COLORS[year])

# Right: TEDn, train=test=year
for year in YEARS:
    probs, targets = load_preds(PRED_ROOT / "TEDn" / f"train_{year}" / f"test_{year}")
    mask = targets == 1
    plot_ecdf(axes[1], 1 - probs[mask], label=str(year), color=YEAR_COLORS[year])
...
plt.tight_layout()
plt.savefig("figure_cdf_comparison.pdf", format="pdf", bbox_inches="tight")
print("Saved: figure_cdf_comparison.pdf")
