...
output_csv = "logging_accuracy_xz_PNU.csv"
...
alpha_dir = None
for seed in seeds:
    model_dir = Path(f"{entrance_path}/normal_sentence/{train_method}/{seed}/{llm}/xy_{epochs}")
    pts = [p for p in model_dir.iterdir()
           if p.is_file() and p.name.lower().endswith(".pt") and train_method in p.name]
    assert len(pts) == 1, f"Expected 1 .pt for {train_method} seed {seed}, found {len(pts)} in {model_dir}"
    model_path = pts[0]
    alpha_dir = model_dir
