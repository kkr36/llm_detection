...
# Evaluate on the v2 parquet (rewrite_X + rewrite_Z_332). model_inference reads this
# env var at call time in get_preds_xy / get_u_data_xy, so no code edit is needed.
os.environ["XY_EVAL_PARQUET"] = "/share/garg/arxiv_kaggle/multillm/data_raw/arxiv_2020_xyz_v2_cs._10000_fronthalf.parquet"
...
# SWITCHES
entrance_path = "/share/garg/arxiv_kaggle/sensitivity_models/xy"
sentence = True
clean = True
epochs = 3
...
n_max_sents = 15000
n_human_sents = 5000
steps = list(range(0, n_max_sents + 1, 2500))  # [0, 2500, ..., 15000]

# Full grid: every (n_x, n_z) cell except (0, 0). This covers the full-range
# heatmap AND the num_X==15000 slice the numZ overlaid line plots require.
experiments = [(x, y) for x in steps for y in steps if not (x == 0 and y == 0)]

# fixed_prompt is just a label ("X"/"Z"); every cell is evaluated once.
configs = [
    {
        "train_method": "TEDn",
        "train_alpha": 0,
        "flip": True,
        "fixed_prompt": "X" if n_x_sents >= n_z_sents else "Z",
        "n_x_sents": n_x_sents,
        "n_z_sents": n_z_sents,
        "llm": f"xz332_nx{n_x_sents}_nz{n_z_sents}_nh{n_human_sents}",
    }
...
output_csv = "logging_accuracy_xz_counts_v2.csv"
...
for cfg in configs:
    ...
    flip         = cfg["flip"]
    llm          = cfg["llm"]
    n_x_sents    = cfg["n_x_sents"]
    n_z_sents    = cfg["n_z_sents"]
    ...
    eval_cols = ["rewrite_X", "rewrite_Z_332"]
    print(cfg, eval_cols)
    ...
    for eval_llm_col in eval_cols:
        ...
        for seed, net in nets.items():
            ...
            save_preds(
                f"{PREDS_BASE}/xz_counts_v2/{train_method}/{llm}/{eval_llm_col}/seed_{seed}.npz",
