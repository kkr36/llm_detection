    ...
    for year in tqdm(years):
        for train_method in train_methods:
            for llm in llm_list:
                for alpha in alphas:
                    # cmd = f"python train_PU_one_year.py --lr=0.00001 --momentum=0 --data-type='llm_type_{llm}' --train-method={train_method} --net-type='DistilBert' --epochs=2 --optimizer=AdamW --alpha=0 --beta=.6 --year={year} --log-dir=logging_accuracy_llm/{llm}_sentence"
                    cmd = f"python train_PU_one_year.py --lr=0.00005 --momentum=0 --data-type='llm_type_{llm}' --train-method={train_method} --net-type='DistilBert' --epochs=3 --optimizer=AdamW --alpha={alpha} --beta=.6 --year={year} --log-dir=logging_accuracy_llm_full_sentence/flip_sentence/alpha_{alpha}/{llm} --clean"
