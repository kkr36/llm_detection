"""
Stage 2 -- join arXiv subject categories.

The parquet `human_abstract` is the front-half of the official abstract, so a
whitespace-normalized 100-char prefix is a stable join key into the full metadata
snapshot. Stream the snapshot once and match both years' sampled abstracts.

Writes ../data/categories.parquet: prefix_key -> categories, primary_category, id.
"""
import json
import pandas as pd
...
keys = set()
for f in ["2020_human_sample.csv", "2025_abstract_sample.csv"]:
