...
if __name__ == "__main__":
    ...
    for train_year in tqdm(train_years):
        logprob_path = f"/share/garg/arxiv_kaggle/alpha/train/logprob_arxiv_tokenized_{train_year}_{category}_{subsample_size}.parquet"
        mle = MLE(logprob_path)

        # do earthmover's dist bt p, q distributions

        # extract logP and logQ arrays in a fixed token order
        tokens = sorted(mle.all_tokens_set)
        logP = np.array([mle.log_p_hat[t] for t in tokens])
        logQ = np.array([mle.log_q_hat[t] for t in tokens])

        # convert to probabilities (safe log-sum-exp normalization)
        P = np.exp(logP - logP.max())
        Q = np.exp(logQ - logQ.max())
        P /= P.sum()
        Q /= Q.sum()

        # Earthmover's distance on a discrete domain
        # Use integer indices as "locations" since tokens are unordered
        emd = wasserstein_distance(np.arange(len(tokens)), np.arange(len(tokens)), u_weights=P, v_weights=Q)
    ...
    plt.xlabel("Year")
    plt.ylabel("Wasserstein Distance P/Q")
    plt.savefig(f"wasserstein.pdf",format='pdf')
    plt.clf()
