...
if __name__ == "__main__":
    ...
    subsample_size = 5000
    train_year = 2020
    logprob_paths = [f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_year}_{train_year}_logprob_{category}_{subsample_size}.parquet", f"/share/garg/arxiv_kaggle/james_analysis/Mapping-the-Increasing-Use-of-LLMs-in-Scientific-Papers/distribution/CS.parquet"]
    labels = ["Our PQ", "James PQ"]

    for logprob_path, label in zip(logprob_paths, labels):
        mle = MLE(logprob_path)

        sols = []
        for year in val_years:
            val_path = f"/share/garg/arxiv_kaggle/val/arxiv_tokenized_{year}_val_{category}_{subsample_size}.parquet"
            solution, half_width = mle.inference(val_path,True)
            print(f"{year}: {solution} += {half_width}")
            sols.append(solution)
        
        # Make a line plot
        plt.plot(val_years, sols, marker='o', label=label)

    # Add labels
    plt.legend(title="Model source", loc="best")
    plt.xlabel("Year")
    plt.ylabel("Pct predicted AI")
    plt.savefig(f"james_us_all.pdf",format='pdf')
    plt.clf()
