...
if __name__ == "__main__":
    ...
    subsample_size = 5000
    train_year = 2020
    logprob_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{train_year}_{train_year}_logprob_{category}_{subsample_size}.parquet"
    # logprob_path = f"/share/garg/arxiv_kaggle/james_analysis/Mapping-the-Increasing-Use-of-LLMs-in-Scientific-Papers/distribution/CS.parquet"
    mle = MLE(logprob_path)
    month_data_path = "/share/garg/arxiv_kaggle/val/month"
    # month_data_path = "/share/garg/arxiv_kaggle/james_analysis/Mapping-the-Increasing-Use-of-LLMs-in-Scientific-Papers/data/inference_data/arxiv/CS"
    # val_path = "/share/garg/arxiv_kaggle/arxiv_tokenized_2010_ai_1000.parquet"
    # print(mle.inference(val_path,True))
    sols = []
    months = []
    # for f in sorted(os.listdir(month_data_path), key=lambda f: tuple(map(int, f.split(".")[0].split("_")))):
    for f in sorted(os.listdir(month_data_path)):
        # if "big" not in f: continue
        val_path = f"/share/garg/arxiv_kaggle/val/month2/{f}"
        # val_path = f"{month_data_path}/{f}"
        month = f.split("_")[2]
        if "big" in f or int(month[:4]) <= 2020 or int(month[:4]) >= 2025: continue
        # month = f.split(".")[0]
        months.append(month)
        # solution, half_width = mle.inference(val_path,True)
        solution, half_width = mle.inference(val_path)
        print(f"{month}: {solution} += {half_width}")
        sols.append(solution)
    
    # Make a line plot
    plt.plot(months, sols, marker='o')
    plt.xticks(months[::5], labels=[str(i) for i in months[::5]])

    # Add labels
    plt.xlabel("Year")
    plt.ylabel("Pct predicted AI")
    # plt.savefig(f"motivating_plot_{train_year}_{category}_{subsample_size}_month2.pdf",format='pdf')
    # plt.savefig("our_direct_repl.pdf", format='pdf')
    plt.savefig("our_pq_our_data.pdf", format='pdf')
    plt.clf()
