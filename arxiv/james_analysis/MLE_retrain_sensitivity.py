...
import os
class MLE():
    def __init__(self, word_df_path, remove_size):
        """
        ...
        self.most_ai = newdf.head(remove_size)['Word'].tolist()
        self.most_human = newdf.tail(remove_size)['Word'].tolist()
        # if not os.path.exists("dfs"):
        #     os.makedirs("dfs")
        # newdf.head(remove_size).to_csv("dfs/most_ai.csv")
        # newdf.tail(remove_size).to_csv("dfs/most_human.csv")
        # import pdb; pdb.set_trace()
    ...
    def inference(self,inference_file_path,remove,exploded_data=False):
        """
        ...
        # Filter out extremes, for sensitivity
        # import pdb; pdb.set_trace()
        if remove == "human":
            inference_data["inference_sentence"] = inference_data["inference_sentence"].apply(
                lambda tokens: [word for word in tokens if word not in self.most_human]
            )
        elif remove == 'ai':
            assert(remove == 'ai')
            inference_data["inference_sentence"] = inference_data["inference_sentence"].apply(
                lambda tokens: [word for word in tokens if word not in self.most_ai]
            )
        else:
            assert(remove=='both')
            inference_data["inference_sentence"] = inference_data["inference_sentence"].apply(
...
if __name__ == "__main__":
    ...
    # val_years = [2020]
    remove_sizes=[0,50,100,200,500][::-1]
    removes = ['human', 'ai', 'both'] # remove either 20 most common ai or human words
    category = "cs."
    subsample_size = 5000
    # train_years = [2010, 2011, 2012, 2013, 2020, 2024]
    train_years = list(range(2010,2026))[1:][::2]
    # import pdb; pdb.set_trace()
    ...
    for remove in removes:
        for remove_size in remove_sizes:
            ...
            for train_year in train_years:
                ...
                mle = MLE(logprob_path,remove_size)

                # for year in val_years:
                val_path = f"/share/garg/arxiv_kaggle/val/arxiv_tokenized_{train_year}_val_{category}_{subsample_size}.parquet"
                solution, half_width = mle.inference(val_path,remove,True)
                print(f"{train_year}: {solution} += {half_width}")
                sols.append(solution)
                
            # Make a line plot
            plt.plot(train_years, sols, marker='o', label=remove_size)

            # Add labels
            plt.legend(title="Number removed tokens", loc="best")
            plt.xlabel("Year")
            plt.ylabel("Pct predicted AI")
            plt.savefig(f"figs/mle_retrain_{remove}_removed.pdf",format='pdf')
        plt.clf()
