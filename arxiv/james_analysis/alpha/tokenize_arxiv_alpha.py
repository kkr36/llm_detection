...
if __name__ == "__main__":
    ...
    for year in tqdm(years):
        ...
        tokenized_val = defaultdict(list)
        ...
        for i, ai_abstract in tqdm(list(enumerate(arxiv_data['ai_abs'][:2000]))):
            tokenized_abs = tokenize(ai_abstract)
            tokenized_train['ai_sentence'] += tokenized_abs
            tokenized_train['ai_index'] += [i for _ in range(len(tokenized_abs))]
            # import pdb; pdb.set_trace()
        for i, human_abstract in tqdm(list(enumerate(arxiv_data['human_abs'][:2000]))):
            tokenized_abs = tokenize(human_abstract)
            tokenized_train['human_sentence'] += tokenized_abs
            tokenized_train['human_index'] += [i for _ in range(len(tokenized_abs))]

        for i, ai_abstract in tqdm(list(enumerate(arxiv_data['ai_abs'][2000:2500]))):
            tokenized_abs = tokenize(ai_abstract)
            tokenized_val['ai_sentence'] += tokenized_abs
            tokenized_val['ai_index'] += [i for _ in range(len(tokenized_abs))]
            # import pdb; pdb.set_trace()
        for i, human_abstract in tqdm(list(enumerate(arxiv_data['human_abs'][2000:2500]))):
            tokenized_abs = tokenize(human_abstract)
            tokenized_val['human_sentence'] += tokenized_abs
            tokenized_val['human_index'] += [i for _ in range(len(tokenized_abs))]

        # tokenized abs --> parquet --> save
        for name, tokenized in [("train", tokenized_train), ("val", tokenized_val)]:
            ...
            # Save to Parquet
            save_path = f"/share/garg/arxiv_kaggle/alpha/{name}/arxiv_tokenized_{year}_{category}_{2000 if name == 'train' else 500}.parquet"
