...
if __name__ == "__main__":
    ...
    category = "cs."
    subsample_size_train = 2000
    subsample_size_val = 500
    train_year = 2010
    val_years = list(range(2010,2026,2))
    alphas = [0, .05, .1, .2, .3, .5][:1]
    sols = {alpha: [] for alpha in alphas}

    for val_year in tqdm(val_years):
        logprob_path = f"/share/garg/arxiv_kaggle/alpha/train/logprob_arxiv_tokenized_{train_year}_{category}_{subsample_size_train}.parquet"
        val_path = f"/share/garg/arxiv_kaggle/alpha/val/arxiv_tokenized_{val_year}_{category}_{subsample_size_val}.parquet"
        val_data_full = pd.read_parquet(val_path)
        ai_sentences, human_sentences = val_data_full['ai_sentence'], val_data_full['human_sentence']

        for alpha in alphas:
            np.random.seed(42)
            mle = MLE(logprob_path)

            # construct val set with alpha
            val_set_size = len(human_sentences) # to keep test set sizes constant per year, replace x % human sentence with llm writing
            val_set_size_human = val_set_size * (1-alpha)
            val_set_size_ai = val_set_size * alpha

            val_set_human = np.random.choice(human_sentences,int(val_set_size_human),replace=False).tolist()
            val_set_ai = np.random.choice(ai_sentences,int(val_set_size_ai),replace=False).tolist()
            val_set_sentences = val_set_human + val_set_ai
            val_data = pd.DataFrame({'inference_sentence': val_set_sentences})
            # import pdb; pdb.set_trace()

            solution, half_width = mle.inference(val_data,True)
            print(f"{val_year} {alpha}: {solution} += {half_width}")
            sols[alpha].append(solution)
    ...
    ax = plt.subplot()
    for alpha in alphas:
        ax.plot(val_years, sols[alpha], label=alpha)
    ...
    plt.xlabel("Year")
    plt.ylabel("% Pred LLM")
    plt.savefig(f"figs/test_alpha_{train_year}.pdf", bbox_inches="tight", format='pdf')
    plt.clf()
