...
if __name__ == "__main__":
    ...
    subsample_sizes = [2000, 500] # train, val sizes
    ...
    for year in tqdm(years):
        for subsample_size in subsample_sizes: 
            ...
            category = "cs."
            name = 'train' if subsample_size == subsample_sizes[0] else 'val'

            train_path = f"/share/garg/arxiv_kaggle/alpha/{name}/arxiv_tokenized_{year}_{category}_{subsample_size}.parquet"
            output_path = f"/share/garg/arxiv_kaggle/alpha/{name}/logprob_arxiv_tokenized_{year}_{category}_{subsample_size}.parquet"
            if os.path.exists(output_path):
                continue

            estimate_text_distribution(train_path, train_path, output_path)