...
model_abbrev_to_id = {
    ...
    "gpt5": "gpt-5",
    "gpt-5": "gpt-5",
    "llama70b": "meta.llama3-70b-instruct-v1:0",
...
def get_completion(
    ...
    **kwargs
    ...
    for attempt in range(max_retries):
        try:
        ...
        except Exception as e: # run bedrock!
            # --- Bedrock path ---
            formatted_prompt = f"""
            ...
            native_request = {
                ...
                "max_gen_len": kwargs.get("max_tokens"),
            ...
            except (ClientError, Exception) as be:
                if attempt == max_retries - 1:  # Last attempt
                    raise be

                wait_time = timeout * (backoff_factor ** attempt)
                if attempt > 0:
                    print(f"Bedrock API error: {be}; retrying in {wait_time:.1f}s... ({attempt + 1}/{max_retries})")
                time.sleep(wait_time)

            # # print(f"Using bedrock due to error: {e}")
