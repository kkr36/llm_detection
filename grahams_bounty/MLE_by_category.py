...
class MLE():
    ...
    def inference(self,inference_data,exploded_data=False):
        ...
        inference_data['inference_sentence'] = inference_data['human_sentence']
...
if __name__ == "__main__":
    ...
    logprob_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_2010_2020_logprob_cs._5000.parquet"
    mle = MLE(logprob_path)

    val_path = f"/share/garg/openreview_data/graham/iclr-dataset/data/tokenized_iclr_2026.parquet"
    val_data = pd.read_parquet(val_path)

    # suppose df['words'] is a column of lists
    flat = [w for row in val_data['keywords'] for w in row]
    counts = Counter(flat)

    n = 30
    top_n = counts.most_common(n)   # n is an int
    print(top_n)

    data = []

    for word, count in tqdm(top_n):
        subset = val_data[val_data['keywords'].apply(lambda lst: word in lst)]
        solution, half_width = mle.inference(subset,True)
        data.append([solution, half_width, word, count])

    import pickle
    with open("by_category.csv", 'wb') as f:
        pickle.dump(data, f)
        
    import pdb; pdb.set_trace()

    # Example: data = [(solution, half_width, word, count), ...]
    solutions  = [t[0] for t in data]
    errors     = [t[1] for t in data]
    words      = [t[2] for t in data]
    counts     = [t[3] for t in data]

    x = np.arange(len(data))

    fig, ax = plt.subplots(figsize=(20, 10))

    bars = ax.bar(x, solutions, yerr=errors, capsize=5)
    ...
    ax.set_ylabel("% Pred AI")
    ax.set_title("Predicted % AI Writing @ ICLR 2026, by common keywords")

    plt.tight_layout()
    plt.savefig("by_category.pdf", format="pdf")


    # solution, half_width = mle.inference(val_path,True)
    # print(f"{year}: {solution} += {half_width}")
