...
if __name__ == "__main__":
    ...
    for year in years:
        ...
        for i, iclr_abstract in tqdm(list(enumerate(iclr_data['abstract'].tolist()[:10000]))):
            # import pdb; pdb.set_trace()
            keywords = iclr_data.iloc[i]['keywords']
            label = iclr_data.iloc[i]['labels']
            tokenized_abs = tokenize(iclr_abstract)
            tokenized['human_sentence'] += tokenized_abs
            tokenized['keywords'] += [keywords for _ in range(len(tokenized_abs))]
            tokenized['labels'] += [label for _ in range(len(tokenized_abs))]
        ...
        # Save to Parquet
        save_path = f"/share/garg/openreview_data/graham/iclr-dataset/data/tokenized_iclr_{year}.parquet"
