...
def estimate_text_distribution(human_source_path, ai_source_path,save_file_path="Word.parquet"):
    ...
    print(human_source_path, ai_source_path)
...
if __name__ == "__main__":
    ...
    for year in tqdm(list(range(start_year,end_year+1))):
        ...
        #     continue
        train_path = f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{year}_{year}_ai_{category}_{subsample_size}.parquet"
    ...
    all_data_path = f"/share/garg/openreview_data/graham/iclr-dataset/data/{'_'.join(years)}.parquet"
    all_data.to_parquet(all_data_path)

    estimate_text_distribution(all_data_path, all_data_path, f"/share/garg/arxiv_kaggle/train/arxiv_tokenized_{start_year}_{end_year}_logprob_{category}_{subsample_size}.parquet")