...
if __name__ == "__main__":
    ...
    import pdb; pdb.set_trace()
    # flat = [w for row in val_data['labels'] for w in row]
    counts = Counter(val_data['labels'].tolist())
    ...
    for word, count in tqdm(top_n):
        try:
            subset = val_data[val_data['labels'] == word]
    ...
    import pickle
    with open("by_label.csv", 'wb') as f:
        pickle.dump(data, f)
    ...
    ax.set_title("Predicted % AI Writing @ ICLR 2026, by label")

    plt.tight_layout()
    plt.savefig("by_label.pdf", format="pdf")
