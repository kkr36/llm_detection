...
class NeuronInterpreter:
    def __init__(
        ...
        n_workers_interpretation: int = 1,
        n_workers_annotation: int = 1,
    ...
    def _execute_prompts(
        ...
        # res = [None] * len(valid_prompts)
        # for i, p in tqdm(list(enumerate(valid_prompts))):
        #     res[i] = self._get_interpretation_openai(p, config)
        # return res
        ...
        for neuron_idx, neuron_interps in interpretations.items():
            ...
            for interp in neuron_interps:
                if interp is None or interp == '':
                    ...
                    continue
                try:
                    annot = [annotations[interp][text] if text in annotations[interp] else 0 for text in neuron_scoring_info['texts']]
                except:
                    import pdb; pdb.set_trace()
